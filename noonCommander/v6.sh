...
paste_action() {
    ...
    local failures=0
    local total_items=0
    local failed_files_tmp
    failed_files_tmp=$(mktemp)

    total_items=$(wc -l < "$CLIPBOARD_FILE" 2>/dev/null || echo 0)
    if (( total_items == 0 )); then show_error "Clipboard empty"; return; fi

    while IFS= read -r src; do
        if [[ -z "$src" || ! -e "$src" ]]; then
            failures=$((failures+1))
            echo "$src" >> "$failed_files_tmp"
            continue
        fi
        ...
        case "$CLIPBOARD_MODE" in
            ...
            cut)
                ...
                ;;
            *)
                show_error "Unknown clipboard mode"; failures=$((failures+1)); echo "$src" >> "$failed_files_tmp" ;;
        esac
    done < "$CLIPBOARD_FILE"
    ...
    if (( any_done )); then
        if [[ "$CLIPBOARD_MODE" == "cut" ]]; then
            if (( failures == 0 )); then
                : > "$CLIPBOARD_FILE" # Clear clipboard on full success
                CLIPBOARD_MODE=""
            else
                # On partial failure, update clipboard with only the failed files
                mv "$failed_files_tmp" "$CLIPBOARD_FILE"
            fi
        fi
        index_dir "$CURRENT_DIR"
        if (( failures == 0 )); then
            show_message "✓ Pasted $total_items item(s)" 0.9
        else
            show_message "Partial paste: $((total_items - failures)) succeeded, $failures failed" 1.2
        fi
    else
...
preview_file() {
    ...
    if [[ -d "$file" ]]; then
        show_error "'$(basename "$file")' is a directory."
        return
    fi
    ...
    case "$mime" in
        text/*)
            if command -v bat &>/dev/null; then
                bat --style=plain --paging=always --color=always "$file" || true
            else
                less "$file" || true
            fi
            ;;
        ...
        application/pdf)
            if command -v pdftotext &>/dev/null; then
                ...
                echo "'pdftotext' not found. Cannot preview PDF."
                sleep 1.5
            fi
            ;;
        audio/*|video/*)
            if command -v ffprobe &>/dev/null; then
                ...
                echo "'ffprobe' not found. Cannot preview media file."
                sleep 1.5
            fi
            ;;
        application/zip)
            if command -v unzip &>/dev/null; then
                ...
                echo "'unzip' not found. Cannot preview zip file."
                sleep 1.5
            fi
            ;;
        *)
    ...
    case "$mime" in
        text/*)
            if [[ -n "${EDITOR:-}" ]]; then
                ...
                show_error "No text editor found."
                sleep 1.5
            fi
            ;;
        *)
            if command -v hexedit &>/dev/null; then
                ...
                show_error "'hexedit' not found. Cannot edit binary file."
                sleep 1.5
            fi
            ;;
    esac
...
System:
  ...
  p          : Preview file (MIME-aware)
  e          : Edit file (text or hex)
  q          : Quit Commander
────────────────────────────────────────────────────────────────
EOF
