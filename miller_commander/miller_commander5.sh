...
navigate_pane() {
    ...
    local pane_height=$((term_height - 2))   # reserve 2 footer lines
    local new_state
    new_state=$("$PANE_MANAGER_SCRIPT" navigate \
        ...
        --cache-file "${pane_ref[cache_file]}" \
        --direction "$direction" \
        --height "$pane_height" 2>/dev/null || true)
    update_pane_state "$1" "$new_state"
}
...
main() {
    ...
    while true; do
        ...
        if [[ "$key" == $'\e' ]]; then
            local seq=""
            while read -rsn1 -t 0.05 char; do seq="$seq$char"; done
            key="$key$seq"
        fi
        ...
        case "$key" in
            ...
            '')   navigate_pane "$ACTIVE_PANE_NAME" "enter" ;;
            ...
            ' ')
                ...
                new_state=$("$PANE_MANAGER_SCRIPT" toggle_mark \
                    ...
                    --scroll "${pane_ref[scroll_offset]}" \
                    --marks-file "${pane_ref[marks_file]}" \
                    --cache-file "${pane_ref[cache_file]}" 2>/dev/null || true)
                update_pane_state "$ACTIVE_PANE_NAME" "$new_state"
                ;;

            $'\e[11~') STATUS_MESSAGE="Help: arrows, Tab to switch, Space to mark, F-keys." ;;
            $'\e[15~') perform_file_operation "copy" ;;
            $'\e[17~') perform_file_operation "move" ;;
            $'\e[18~') perform_file_operation "delete" ;;
            $'\e[21~'|'q') break ;;
        esac
    done
}
